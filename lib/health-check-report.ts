...
function findReference(rules: RuleRef[], sources: SourceRef[], terms: string[]) {
  ...
  return sourceMatch?.url ?? "No official source reference was retrieved for this check.";
}
...
function buildChecks(params: { business: BusinessInfo; docs: ReportDocument[]; rules: RuleRef[]; sources: SourceRef[] }) {
  ...
  checks.push({
    ...
    source_reference: findReference(rules, sources, ["waste transfer note business waste gov.uk", "waste duty of care gov.uk"])
  });
  ...
  checks.push({
    ...
    source_reference: findReference(rules, sources, ["waste carrier licence environment agency", "search waste carriers brokers"])
  });
  ...
  checks.push({
    ...
    source_reference: findReference(rules, sources, ["waste carrier licence environment agency", "public register waste carriers"])
  });
  ...
  checks.push({
    ...
    source_reference: findReference(rules, sources, ["dispose business commercial waste", "waste transfer note"])
  });
  ...
  checks.push({
    ...
    source_reference: findReference(rules, sources, ["waste duty of care gov.uk", "dispose business commercial waste"])
  });
  ...
  if (business.produces_food_waste) {
    ...
    checks.push({
      ...
      source_reference: findReference(rules, sources, ["food waste workplace recycling england gov.uk", "simpler recycling workplace recycling"])
    });
  }
  ...
  if (business.produces_hazardous_waste) {
    checks.push({
      ...
      source_reference: findReference(rules, sources, ["hazardous waste consignment note gov.uk", "hazardous waste note"])
    });
  }

  const wtnWithDestination = wtDocs.filter((d) => lower(`${d.ai_summary}`).includes("destination"));
  checks.push({
    check_name: "Waste destination present on WTN where available",
    result: wtDocs.length === 0 ? "cannot_verify" : wtnWithDestination.length ? "pass" : "attention_needed",
    evidence_used: wtnWithDestination.map((d) => d.file_name),
    affected_document: wtDocs[0]?.file_name ?? null,
    recommended_action: wtnWithDestination.length ? "No immediate action." : "Ensure destination details are visible on waste transfer records.",
    source_reference: findReference(rules, sources, ["waste transfer note business waste gov.uk", "waste duty of care gov.uk"])
  });
  ...
  checks.push({
    ...
    source_reference: findReference(rules, sources, ["ewc code waste transfer note", "waste transfer note guidance"])
  });
  ...
  if (failedDocs.length > 0) {
    checks.push({
      ...
      source_reference: "No official source reference was retrieved for this check."
    });
  }
...
function scoreFromChecks(params: { checks: BaselineCheck[]; docs: ReportDocument[]; business: BusinessInfo }) {
  ...
  if (byName("Carrier licence valid / not expired")?.result === "fail") deductions.push({ reason: "Expired carrier licence", points: 30 });
  if (byName("Carrier licence evidence present")?.result === "fail") deductions.push({ reason: "Missing carrier licence evidence", points: 25 });
  if (business.produces_food_waste && byName("Food waste evidence present")?.result === "fail") deductions.push({ reason: "Missing food waste evidence", points: 20 });
  ...
  if (byName("Supplier/contract evidence present")?.result === "attention_needed") deductions.push({ reason: "Missing supplier contract evidence", points: 10 });
  if (byName("Waste destination present on WTN where available")?.result === "attention_needed") deductions.push({ reason: "Missing destination detail on WTN", points: 10 });
  if (byName("EWC code present on WTN where available")?.result === "attention_needed") deductions.push({ reason: "Missing EWC code on WTN", points: 10 });
  ...
  const checksWithoutRef = checks.filter((c) => c.source_reference.startsWith("No official source reference")).length;
  if (checksWithoutRef > 0) deductions.push({ reason: "Missing official references for some checks", points: Math.min(10, checksWithoutRef) });
...
function confidenceFromSignals(params: {
  ...
  const noRefChecks = checks.filter((c) => c.source_reference.startsWith("No official source reference")).length;
...
export async function buildHealthCheckReportForBusiness(params: { businessId: string; userId: string }): Promise<HealthCheckReport> {
  ...
  checks.filter((c) => c.source_reference.startsWith("No official source reference")).forEach((c) => cannotVerify.add(`${c.check_name}: no official source reference was retrieved.`));
