...
import { getEditableActiveHealthCheck } from "@/lib/health-checks";
...
export async function POST(request: Request) {
  ...
  for (const file of files) {
    try {
      const document = await uploadDocument({ userId: user.id, businessId: business.id, file, healthCheckId: activeHealthCheck.id });
