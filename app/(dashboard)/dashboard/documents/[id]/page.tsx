...
export default async function DocumentDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: doc } = await supabase
    .from("documents")
    .select(
      "id,file_name,created_at,processing_status,processing_error,document_type,extracted_supplier,extracted_date,expiry_date,waste_type,extracted_ewc_code,extracted_licence_number,ai_summary,ai_risk_level,ai_extracted_json,health_check_id"
    )
    .eq("id", params.id)
    .eq("user_id", user.id)
  ...
  if (doc.health_check_id) {
    ...
    canEdit = Boolean(hc && hc.status === "active" && !hc.locked_at);
  }
  ...
  return (
    <div className="space-y-4">
      <section className="app-panel p-5">
        ...
        {!canEdit ? (
          <p className="mt-2 text-xs text-amber-700">This document belongs to a locked health check and is read-only.</p>
        ) : null}
      </section>
